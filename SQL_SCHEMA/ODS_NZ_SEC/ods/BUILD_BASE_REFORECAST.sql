...
-- =============================================
CREATE PROCEDURE [ods].[BUILD_BASE_REFORECAST]
...
BEGIN
	...
	FROM (SELECT
		...
		('Base Build') as [source]
	FROM ods.FCT_GLBUDGET_NEW glt
	INNER JOIN ods.DIM_GT_PERIOD pr ON glt.periodOid = pr.oid
	INNER JOIN ods.DIM_GT_GL gl on glt.glOid = gl.oid
	INNER JOIN ods.DIM_GT_COA gtCoa on gl.coaID = gtCoa.id
	INNER JOIN ods.DIM_AX_COA axCOA on gtCoa.axGLAcc = axCOA.axGLAcc
	LEFT JOIN ods.DIM_JOB jo ON glt.jobOid = jo.oid
	--LEFT JOIN ods.DIM_ORGANISATION orn ON glt.orgOid = orn.oid
	WHERE pr.yearNo = @forecastYear
	AND pr.mnthNo > @actualQtrEnd
	AND axCOA.glReport = 'PNL'
	GROUP BY pr.stDate , jo.code, gl.accountNo) glExt
