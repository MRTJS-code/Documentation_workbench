...
DELETE FROM metadata.CTL_JOB_CONFIG_VARIABLES WHERE JOB_CODE IN (8005,8006) -- to remove existing variables if repurposing an existing run code
...
INSERT INTO [metadata].[CTL_JOB_CONFIG_VARIABLES]
     ...
     VALUES
           ...
           ,'ETL_CUS_REBUILD_DATE'									--variable code
           ,'nvarchar'												--data type (int, datetime, nvarchar)
           ,'Effective from date for FIXED rebuild'					--variable description
           ,'2025-11-24'											--variable value
           ,sysdatetime()											--date last modified
...
INSERT INTO [metadata].[CTL_JOB_CONFIG_DEPENDENCIES]
           ...
           ,[DATE_LAST_MODIFIED])
     VALUES
           (8005			--job code to wait
           ,8006			--job code it is waiting to run
...
UPDATE [metadata].[CTL_JOB_CONFIG]
   SET [JOB_NAME] = 'Job DW NZ Deputy Fixed'
      ,[JOB_STATUS] = 'PAUSED'
      ,[JOB_INTERVAL_MODE] = 'DAILY'
      ,[JOB_INTERVAL_VAL] = 1
      ,[JOB_DATE_KICK_OFF] = '2025-12-02 00:00:00'
      ,[DATA_RANGE_MODE] = 'FIXED'
      ,[DATA_RANGE_FIXED_INTERVAL_MODE] = 'DAILY'
      ,[DATA_RANGE_FIXED_INTERVAL_VAL] = 1
      ,[DATA_RANGE_INC_TIME] = 'NO'
      ,[DATA_RANGE_DAYS_DELAY] = 0
      ,[DATA_RANGE_DATE_START] = '2025-12-02 00:00:00'
      ,[NOTIF_AFTER_CONS_FAILURE] = 1
      ,[NOTIF_AFTER_RUNNING_MINS] = 30
      ,[NOTIF_EMAIL_ADDRESS] = 'tony.smith@firstsecurity.co.nz'
      ,[ERROR_AFTER_CONS_FAILURE] = 1
      ,[ETL_INTEGRATED] = 'YES'
      ,[ETL_FOLDER_NAME] = 'Operational Data Store'
      ...
      ,[ETL_RUN_32BIT_MODE] = 'NO'
      --,[JOB_EXCLUSION_TIME_START] = '00:00'
      --,[JOB_EXCLUSION_DURATION_HRS] = 1
 --     ,[JOB_EXCLUSION_DAYS] = <JOB_EXCLUSION_DAYS, varchar(50),>
 --     ,[JOB_SUCCESSIVE_GROUPS] = <JOB_SUCCESSIVE_GROUPS, varchar(100),>
      ,[JOB_DELTA_USE_RUN_DATE] = 'YES'
 WHERE JOB_CODE = 8006
GO
...
SELECT * FROM metadata.[CTL_JOB_CONFIG] WHERE JOB_CODE IN (8005,8006)

SELECT *
  FROM [metadata].[CTL_JOB_CONFIG_DEPENDENCIES]
  WHERE [JOB_DEPENDER_CODE] IN (8005,8006)
  OR [JOB_DEPENDEE_CODE] IN (8005,8006)

SELECT * FROM metadata.[CTL_JOB_CONFIG_VARIABLES] WHERE JOB_CODE IN (8005,8006)