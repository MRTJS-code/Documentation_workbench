...
- Summarize tables from `/SQL_SCHEMA/DataIntegration/eda` or `/SQL_SCHEMA/ODS_NZ_SEC/{schema name}` and relate them to package actions; explicitly mark anything inferred as assumptions/gaps.
...
- `/SQL_SCHEMA/DataIntegration/eda` files provided definitive table definitions and views (no guessing on columns).  When using ODS_NZ_SEC schemas are dw, staging and lookup.
...
## Gotchas from the DPDISSIS Repo
