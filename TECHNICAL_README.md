...
- The enterprise **ETL Framework** (SSIS package `ETL Framework.dtsx`, scheduled via SQL Agent) runs about every 1–2 minutes on-premises and orchestrates all jobs.
...
- Key tables:
  ...
  - `ERR_MAX_FAIL` and `ERR_PRIOR_FAIL_COUNT` are sourced from job config and drive retry vs error behavior in the EDA OnError handler.
...
- **1 Lookup Greentree.dtsx**: Builds lookups referenced by later transformations from the data warehouse (Greentree is dormant; package will be renamed later).
...
- **5 Billing Extract.dtsx**: Generates and emails a billing CSV extract for D365, then creates a new event into `eda.ETL_EVENT` targeting Deputy (output DEPUTY, input GENERAL RESOURCE) and inserts invoice JSON batches into `eda.IMP_DEPRESOURCE` from `eda.EXT_DEP_FULL` (filters PayApproved, product codes `SG%`/`RBSG%` when extract type is not `All`).
- **5 Payroll Extract.dtsx**: Generates and emails a payroll CSV extract, then creates two new events—(a) mark exported timesheets via `IMP_DEPRESOURCE` (output DEPUTY, input GENERAL RESOURCE); (b) queue paid events per pay period (input TIMESHEET PAID) with grouped `TimesheetIdArray` payloads.
...
## Operational Notes
- Trigger: External process (or scripts in `/BCP SQL Scripts`) inserts into `eda.ETL_EVENT`; `0 Job Plan` is executed by the ETL Framework SQL Agent job (every ~1–2 minutes) using job metadata (job code 6002) from `metadata.CTL_JOB_CONFIG`.
...
- Concurrency/backoff: OnError uses `ERR_MAX_FAIL`/`ERR_PRIOR_FAIL_COUNT` from underlying ETL job config to decide `RETRY` vs `ERROR`.
- Dead Letter Logic: Once an event reaches an `ERROR` state the processor will not process any other events.  Errors will be reviewed and resolved first, then an administrator will manually set `EVENT_STATUS` back to `RETRY` when the underlying ETL Job is operational again.  Future enhancements will manage this cleanly should other events appear for an EDA processor in an error state.
...
- Typical run cadence: ETL Framework job every ~1–2 minutes; per-event runtime depends on Deputy API and extract sizes (not specified in repo).
